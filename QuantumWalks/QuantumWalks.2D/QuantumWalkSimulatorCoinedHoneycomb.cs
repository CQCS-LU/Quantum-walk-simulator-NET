...
namespace CQCS.QuantumWalks.Grid2D
{
    ...
    /// </summary>
	public class QuantumWalkSimulatorCoinedHoneycomb : IQuantumWalkSimulator2D
    {
        ...
        public QuantumWalkSimulatorCoinedHoneycomb(int height, int width, Coin coin = Coin.AKR)
        {
            Height = height;
            Width = width;
        ...
        private void GroverCoin(int x, int y)
        {
            if ((x + y) % 2 == 0)
            {
                double sum = state[x, y, Left] + state[x, y, UpRight] + state[x, y, DownRight];
                double avg = sum / 3;
            ...
            {
                double sum = state[x, y, Right] + state[x, y, UpLeft] + state[x, y, DownLeft];
                double avg = sum / 3;
		...
		public double GetScalarProduct()
		{
			...
			for (int x = 0; x < Width; x++)
			{
                if ((x + y) % 2 == 0)
                {
                    amplitudeSum += state[x, y, Left] + state[x, y, UpRight] + state[x, y, DownRight];
                }
                else
                {
                    amplitudeSum += state[x, y, Right] + state[x, y, UpLeft] + state[x, y, DownLeft];
                }
            }
