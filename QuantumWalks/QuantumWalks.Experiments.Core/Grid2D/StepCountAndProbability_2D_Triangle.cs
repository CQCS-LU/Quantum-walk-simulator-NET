...
{
    /// <summary>
    /// Numerical experiments on triangular 2D grid with multiple marked vertices.
    /// </summary>
	public static class StepCountAndProbability_2D_Triangle
    {
        ...
        private static void OptimalWeight(int gridSize, int m)
        {
            ...
            for (decimal a = m-1; a <= m+1; a += 0.01m)
            {
                double selfLoopWeight = 6.0 * (double)a / N;

                var qws = new CQCS.QuantumWalks.Grid2D.Lackadaisical.QuantumWalkSimulatorTriangle(gridSize, gridSize, selfLoopWeight: selfLoopWeight);
